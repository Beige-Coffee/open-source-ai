#!/usr/bin/env node
// Batch entailment verifier for AA chunk 4.
// Mirrors run-aa-chunk-1.mjs: per row, runs `audit:verify:show` to load
// the claim + snapshot, parses the relevant numeric field for each row
// kind (bench / cost / speed), compares, and runs `audit:verify:update`
// with verdict supported / contradicted / verifier_unable.

import { execFileSync } from 'node:child_process';
import { readFileSync, writeFileSync } from 'node:fs';

const CHUNK_PATH = '/tmp/aa-verify-chunk-4.txt';
const REPORT_PATH = '/tmp/aa-verify-chunk-4-report.md';
const LOG_PATH = '/tmp/aa-verify-chunk-4-log.txt';
...
function runShow(id) {
  return execFileSync(
    ...
    { encoding: 'utf8', cwd: '/Users/austinv2/code/open-source-ai-stack', maxBuffer: 16 * 1024 * 1024 }
  );
}
  ...
  switch (kind) {
    ...
    case 'speed-output': {
      const m = claim.match(/output\s+throughput[^0-9]*([\d.]+)\s*tokens?\s*\/\s*sec/i)
        ...
        || claim.match(/output\s+speed[^0-9]*([\d.]+)\s*tokens?\s*\/\s*sec/i)
...
for (const id of ids) {
  ...
  try {
    ...
    if (!kind) {
      result = { id, verdict: 'verifier_unable', evidence: '', notes: `non-AA row kind; snapshot not AA model page` };
    } else if (!snapshot) {
...
const lines = [];
lines.push(`# AA verify chunk 4 report`);
lines.push('');
