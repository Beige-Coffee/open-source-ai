#!/usr/bin/env node
// Batch entailment verifier for AA chunk 3.
// For each row id:
...
//
// Adapted from run-aa-chunk-1.mjs. Chunk-3 contains some non-AA lineage
// rows whose source URL is not artificialanalysis.ai; those are handled
// as `verifier_unable` because the AA-snapshot pattern does not apply.

import { execFileSync } from 'node:child_process';
import { readFileSync, writeFileSync } from 'node:fs';

const CHUNK_PATH = '/tmp/aa-verify-chunk-3.txt';
const REPORT_PATH = '/tmp/aa-verify-chunk-3-report.md';
const LOG_PATH = '/tmp/aa-verify-chunk-3-log.txt';
...
for (const id of ids) {
  ...
  try {
    ...
    if (kind === 'lineage') {
      // Lineage rows in this chunk carry a "field removed (because ...)"
      // meta-justification claim whose source is the model's launch post,
      // not an AA model page. Mark verifier_unable.
      result = {
        id,
...
const lines = [];
lines.push(`# AA verify chunk 3 report`);
lines.push('');
